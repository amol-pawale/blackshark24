<!-- ======================= HEADER ======================= -->
<div align="center">

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=26&pause=1000&color=512BD4&center=true&vCenter=true&width=600&lines=Hi+%F0%9F%91%8B+I'm+Amol+Pawale;Senior+Software+Engineer;Backend+Architecture+%E2%9A%99%EF%B8%8F;Salesforce+Integrations+%E2%98%81%EF%B8%8F;Aspiring+Solutions+Architect+%F0%9F%8F%97%EF%B8%8F" alt="Typing SVG" />
</a>

<p>
  <img src="https://komarev.com/ghpvc/?username=amol-pawale&label=Profile%20views&color=512BD4&style=flat" alt="profile views" />
</p>

</div>

<!-- ======================= INTRO ======================= -->

### 🚀 Building secure, high-performance backend architectures

I'm a **Senior Software Engineer** with **4.5+ years** building scalable enterprise backend systems and seamless CRM integrations. My foundation is architecting robust **RESTful APIs**, managing databases, and routing traffic with **.NET Core, .NET Framework, and Kong API Gateway**.

Over the last **18 months** I've expanded into the **Salesforce ecosystem** (Apex, Lightning Web Components), where my backend depth lets me tackle complex integration challenges — **secure multi-tenant authentication systems** and **high-volume email processing pipelines** among them. Whether I'm optimizing a SQL query, configuring Kong, or refactoring an LWC, the goal is the same: architectures that bridge heavy backend systems with modern enterprise platforms.

- 🔭 &nbsp;Currently building **Commerce** — a .NET 10 Clean Architecture backend
- 🌱 &nbsp;Currently learning **System Design · AWS · GenAI / RAG**
- 🎯 &nbsp;Working toward a **Solutions Architect** role
- 📫 &nbsp;Reach me at **pawale024@gmail.com**
- ⚡ &nbsp;Fun fact: I love **gaming and anime**

<br/>

<!-- ======================= FEATURED PROJECTS ======================= -->

## 🌟 Featured Projects

### 🛒 Commerce
![Stack](https://img.shields.io/badge/Stack-.NET_10_·_Clean_Architecture-512BD4?style=flat-square)
![Status](https://img.shields.io/badge/Status-In_Development-orange?style=flat-square)

**Production-grade e-commerce backend built on Clean Architecture**

A portfolio-grade backend showcasing enterprise architecture patterns end to end — domain-driven Clean Architecture layering, async messaging, caching, and API gateway routing, all containerized and observable.

**⚙️ How it's built:**
- 🧱 &nbsp;**Clean Architecture** — strict domain / application / infrastructure separation on **.NET 10**
- 🐘 &nbsp;**PostgreSQL + EF Core** — Code-First migrations as the persistence layer
- 🐇 &nbsp;**RabbitMQ** — async event-driven messaging between services
- ⚡ &nbsp;**Redis** — caching layer for high-throughput reads
- 🚪 &nbsp;**Kong API Gateway** — routing, rate limiting, and edge policy
- 📊 &nbsp;**OpenTelemetry + Grafana Cloud** — traces, metrics, and logs
- 🔄 &nbsp;**GitHub Actions CI/CD** — self-hosted runner with automated PR review

<br/>

<!-- 🔧 PLACEHOLDER: duplicate this block for each additional project you want to feature.
     Fill in: title, badges, one-line tagline, description, and bullet points. -->

### 🧩 Project Name
![Stack](https://img.shields.io/badge/Stack-Tech_·_Tech-4169E1?style=flat-square)
![Status](https://img.shields.io/badge/Status-Live-brightgreen?style=flat-square)

**One-line tagline describing what it does**

Short paragraph describing the project, the problem it solves, and the interesting technical bits.

**🎯 Highlights:**
- 🔹 &nbsp;Key feature or capability #1
- 🔹 &nbsp;Key feature or capability #2
- 🔹 &nbsp;Key feature or capability #3

<br/>

<!-- 🔧 PLACEHOLDER: another project slot -->

### 🧩 Project Name
![Stack](https://img.shields.io/badge/Stack-Tech_·_Tech-00A1E0?style=flat-square)
![Status](https://img.shields.io/badge/Status-WIP-yellow?style=flat-square)

**One-line tagline describing what it does**

Short paragraph describing the project, the problem it solves, and the interesting technical bits.

**🎯 Highlights:**
- 🔹 &nbsp;Key feature or capability #1
- 🔹 &nbsp;Key feature or capability #2
- 🔹 &nbsp;Key feature or capability #3

<br/>

<!-- ======================= TECH STACK ======================= -->

## 🛠️ Tech Stack & Tools

**⚙️ Backend & Architecture**

![.NET](https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)
![.NET Framework](https://img.shields.io/badge/.NET_Framework-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=csharp&logoColor=white)
![Kong](https://img.shields.io/badge/Kong-003459?style=for-the-badge&logo=kong&logoColor=white)
![RabbitMQ](https://img.shields.io/badge/RabbitMQ-FF6600?style=for-the-badge&logo=rabbitmq&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)

**🗄️ Data Layer**

![SQL Server](https://img.shields.io/badge/SQL_Server-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white)

**☁️ Salesforce Ecosystem**

![Salesforce](https://img.shields.io/badge/Salesforce-00A1E0?style=for-the-badge&logo=salesforce&logoColor=white)
![Apex](https://img.shields.io/badge/Apex-00A1E0?style=for-the-badge&logo=salesforce&logoColor=white)
![LWC](https://img.shields.io/badge/Lightning_Web_Components-00A1E0?style=for-the-badge&logo=salesforce&logoColor=white)

**🔧 DevOps · Observability · Currently Learning**

![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white)
![Grafana](https://img.shields.io/badge/Grafana-F46800?style=for-the-badge&logo=grafana&logoColor=white)
![OpenTelemetry](https://img.shields.io/badge/OpenTelemetry-425CC7?style=for-the-badge&logo=opentelemetry&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)

<br/>

<!-- ======================= CONNECT ======================= -->

## 🤝 Let's Connect

<p align="left">
  <a href="https://linkedin.com/in/amol24" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="https://dev.to/blackshark24" target="_blank">
    <img src="https://img.shields.io/badge/Dev.to-0A0A0A?style=for-the-badge&logo=devdotto&logoColor=white" alt="Dev.to" />
  </a>
  <a href="mailto:pawale024@gmail.com">
    <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
</p>

<br/>

<!-- ======================= GITHUB ANALYTICS ======================= -->

## 📊 GitHub Analytics

<div align="center">

<a href="https://github.com/amol-pawale">
  <img height="165" src="https://github-readme-stats-sigma-five.vercel.app/api?username=amol-pawale&show_icons=true&theme=react&hide_border=true&bg_color=0D1117&title_color=512BD4&icon_color=512BD4&count_private=true" alt="Amol's GitHub Stats" />
</a>
<a href="https://github.com/amol-pawale">
  <img height="165" src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=amol-pawale&layout=compact&theme=react&hide_border=true&bg_color=0D1117&title_color=512BD4&langs_count=8" alt="Top Languages" />
</a>

<br/>

<img src="https://streak-stats.demolab.com?user=amol-pawale&theme=react&hide_border=true&background=0D1117&ring=512BD4&fire=512BD4&currStreakLabel=512BD4" alt="GitHub Streak" />

<br/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=amol-pawale&theme=react-dark&hide_border=true&bg_color=0D1117&color=512BD4&line=512BD4&point=ffffff" alt="Activity Graph" />

</div>

<br/>

<div align="center">
  <i>⭐ "Building secure, high-performance systems — one architecture decision at a time."</i>
</div>
